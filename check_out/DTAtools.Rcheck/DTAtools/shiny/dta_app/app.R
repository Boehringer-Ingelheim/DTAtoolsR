...
ui <- bslib::page_fluid(
  ...
  # Hidden download button for export modal
  div(style = "display: none;", downloadButton("export_trigger_download", ""))
)
    ...
    tryCatch(
      {
        if (input$export_format == "markdown") {
          ...
          if (isTRUE(input$export_as_pdf)) {
            ...
            if (requireNamespace("rmarkdown", quietly = TRUE) &&
              ...
              rmarkdown::pandoc_convert(input = output_file, to = "pdf", output = pdf_file)
              output_file <- pdf_file
              filename <- sub("\\.md$", ".pdf", filename)
            } else {
              showNotification(
                "Pandoc not available. Exporting as Markdown instead.",
                type = "warning",
                duration = 10
              )
            }
        ...
        shinyjs::click("export_trigger_download")
