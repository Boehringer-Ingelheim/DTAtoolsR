...
test_that("clinical_data_error_columnspec.csv triggers column spec failures and one import failure", {
  dta <- load_clinical_fixture_dta("clinical_data_error_columnspec.csv")

  res <- results(dta)
  table_name <- res$target[1]
  details <- validation_errors(dta[["clinical_data"]], table = table_name, source = "memory")
  columnspec_full <- as.data.frame(details$columnspec_errors$full_error)
  msgs <- messages(dta, as_tibble = FALSE)

  expect_equal(nrow(res), 1)
  expect_equal(res$dataset, "clinical_data")
  expect_equal(res$target, "clinical_data_error_columnspec")
  expect_equal(res$status, "failed")
  expect_gt(res$n_columnspec_errors, 0)
  ...
  # The table is invalid on the three-axis verdict, not merely on the column spec
  # axis: the column spec count is now lower than it was, and `ok` must not follow it.
  expect_false(details$ok)
  expect_equal(sort(unique(columnspec_full$keyword)), c("const", "enum", "maxLength", "required", "type"))
  expect_equal(sort(unique(columnspec_full$column)), c("BMI", "GENDER", "STUDYID", "VISIT"))
  ...
  # axis. The column spec count falls by ~500; `ok` is unmoved because the import axis
  # carries the failure.
  bmi_columnspec <- columnspec_full[columnspec_full$column %in% "BMI", , drop = FALSE]
  expect_equal(nrow(bmi_columnspec), 1)
  expect_equal(bmi_columnspec$keyword, "type")
  ...
  expect_equal(sort(unique(msgs$source)), c("columnspec", "import"))
...
test_that("clinical_data_error_all.csv combines column spec, rule and import failures", {
  ...
  # added on rows the pre-existing column spec/rule edits did not touch.
  ...
  # the 4 unconvertible cells becoming NA is also a column spec `type` violation --
  ...
  # That fifth cell is the unavoidable column-spec axis fallout: every numeric
  # column in this spec is non-nullable, so a missing value there can never be
  # column-spec-clean, even though it is import-clean.
  expect_equal(sort(unique(columnspec_full$keyword)), c("const", "enum", "maxLength", "type"))
  expect_equal(
    sort(unique(columnspec_full$column)),
...
test_that("clinical_data_error_import.csv triggers import failures, isolated as far as the spec allows", {
  ...
  # WEIGHT ">300") becomes NA and is *also* a column spec `type` violation -- plus
  # one more `type` violation for a genuinely missing (blank) BMI cell that
  # contributes 0 import errors. This is the unavoidable column-spec axis fallout
  ...
  expect_equal(res$n_columnspec_errors, 5)
  ...
  expect_equal(sort(unique(columnspec_full$keyword)), "type")
  expect_equal(sort(unique(columnspec_full$column)), c("AGE", "BMI", "HEIGHT", "WEIGHT"))
