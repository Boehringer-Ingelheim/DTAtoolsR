...
S7::method(results, DTADataSetTabular) <- function(x, tables = NULL) {
  validation_status(x, tables = tables)
}
...
S7::method(results, DTADataSet) <- function(x, tables = NULL) {
  ...
  # Reuse the existing tabular status formatter.
  validation_status(x, tables = tables)
}
...
S7::method(results, DTA) <- function(x, datasets = NULL) {
  ...
  rows <- lapply(target_datasets, function(ds_name) {
    ...
    data.frame(
      dataset = ds_name,
      n_tables = nrow(status),
      n_validated = sum(status$status == "validated", na.rm = TRUE),
      n_valid = sum(status$ok == TRUE, na.rm = TRUE),
      n_invalid = sum(status$ok == FALSE, na.rm = TRUE),
      n_skipped = sum(status$status == "skipped", na.rm = TRUE),
      n_not_validated = sum(status$status == "not_validated", na.rm = TRUE),
      stringsAsFactors = FALSE
    )
  })
